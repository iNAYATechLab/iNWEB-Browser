...
| ADR-042 | 2026-09-25 | Extension subsystem: documented deviation from §16 — the pinned Android baseline does not build the extensions platform (extensions/buildflags/buildflags.gni: enable_extensions = !is_android && !is_ios && !is_castos && !is_fuchsia, and enable_extensions_core = enable_extensions || enable_desktop_android_extensions; our args are target_os="android" with no override, so both are false), so §16 is preserved as behavioural/specification intent and not implemented. No extension UI or runtime binding, and no placeholder: a settings screen whose enable does nothing is a fake feature (§57). Patch ids 0014-0017 reserved and unused so the series can resume with its identity; src/core/extensions (17 tests, ADR-023) stays the behavioural spec, as ADR-040 kept the Kotlin engine as the spec for the C++ port. Re-evaluation: upstream enables extensions on Android, a future pin exposes a supported Android path, or the baseline changes. A limited user-script/content-injection capability is explicitly NOT started here and needs its own spec/ADR |
...
- **Stage 2 in progress** — iNWEB patch series: **0001–0012 authored**
  ...
  ci-authoring 35763178530 green. 0009 (tab-takeover redirect throttle —
...
Roles assigned by the project owner: **Lead Developer** = build/CI agent
(architecture, Chromium integration, core browser functionality,
build + CI ownership); **Junior Developer** = Home Page UI/UX only.

- Ownership map, conflict rules, review protocol and the standing
  architecture decisions (D1–D5) live in
  [`docs/TEAM-WORKFLOW.md`](docs/TEAM-WORKFLOW.md).
- The Junior's onboarding + Home Page build order + review checklist live in
  [`docs/HOME-PAGE-HANDOFF.md`](docs/HOME-PAGE-HANDOFF.md).
- Key decisions: Home Page logic is a new pure-JVM module `src/core/home`
  (testable today, CI-enforced); Chromium-tree code for it ships as patch
  entry `ui/0026-home-page`, generated by the Lead (0025 stays reserved for
  the adult-content blocker); the design prototype on
  `feature/home-prototype` is the Junior's and is not touched by the Lead;
  no PR workflow — branch diff review, then the Lead lands it on `main`.
