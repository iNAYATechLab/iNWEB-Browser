...
| 7 | Compromised extension | registry core implemented; enforcement patch-side — **G-10** | Extension registry state machine + permission-review records (17 tests); sideload-first, no silent installs, manual updates (ADR-023); scoped API surface + kill-switch ship with patches 0014–0017 |
| 8 | Credential theft on device | partial — **G-03** | VPN secrets opaque by construction (`SecretValue`, ADR-025); backup secret guard (`KNOWN_STORES`, ADR-026); Keystore/BiometricPrompt envelope = patch 0021, pending |
...
| 11 | Profile leakage | isolation contract implemented; binding pending — **G-05** | Namespace isolation is structural in the registry (12 tests, ADR-026); per-profile data directories bind via patch 0023 |
...
| G-03 | Keystore/BiometricPrompt binding | Secrets are opaque in cores; at-rest encryption + biometric gating bind via patch 0021 (ADR-025 envelope design) | patch 0021, B-001 |
...
| G-05 | Per-profile directories | The namespace contract is structural and tested; directory binding is patch 0023 | patch 0023, B-001 |
...
| G-10 | Extension enforcement surface | Registry/permission-review core done; scoped API surface + kill-switch enforcement ship with patches 0014–0017 | patches 0014–0017, B-001 |
