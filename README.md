...
| Patch framework | `iNWEB_PATCHES/` registry + apply/verify/hash tooling — tested; series 0001–0003 CI-verified on the real pinned tree (b001-verify run 35739593282: `PATCHES: PASS`) |
| CI | **Live**: Python (80 tests) + registry + storage-inventory gate + string parity incl. the Chromium-baseline mirror + UI externalization gate; Kotlin core (414 tests, 11 modules, cross-module `--deps` support) + authored-source structural gate; weekly upstream watch; benchmark on demand (not in CI); Stage-2 gates live: `b001-verify` (series verification on the real fetched tree, push-triggered) + `b001-build-hop` (resumable chained builds, dispatch-only) | |
| Build | **PROVEN & RELEASED** — v1.0.0-alpha.1 (pristine engine baseline, 689 MB APK, sha256 verified runner → artifact → release) built on GitHub-hosted runners via the 14-run resumable B-001 chain (evidence: `docs/verification/B001-BUILD-CHAIN.md`); Stage-2 incremental builds resume from cached state-13 |
...
│   ├── phases/                     # 14 phase design documents
│   ├── design/                     # Brand & logo docs + study assets
│   ├── releases/                   # Per-version release notes + template
│   ├── verification/               # B-001 build-chain evidence
│   └── … (governing / security / QA docs — full index in docs/README.md)
...
Full index with per-document purposes: [`docs/README.md`](docs/README.md).

| Group | Where |
|---|---|
| Governing (spec, protocol, versioning, licensing) | `docs/` root |
| Architecture & build infrastructure | `docs/` root |
| Phase designs (14) | [`docs/phases/`](docs/phases/) |
| Security & privacy | `docs/` root |
| QA, budgets & audits | `docs/` root |
| Brand & design | [`docs/design/`](docs/design/) |
| Release notes | [`docs/releases/`](docs/releases/) |
| B-001 verification evidence | [`docs/verification/`](verification/) |
...
2. **Build environment (GitHub-hosted, chained Actions runners — proven).** Chromium
   compiles through resumable chained jobs (`b001-build-hop.yml`) with cached build
   state; v1.0.0-alpha.1 was produced and released this way (evidence:
   `docs/verification/B001-BUILD-CHAIN.md`). A larger self-hosted runner remains an
   optional accelerator, not a dependency; `fetch_chromium.sh` / `build_android.sh` /
   the Docker image stay as build-host alternatives.
