...
| Core module (`src/core/browser-shell`) | **Implemented & unit-tested — 101 Kotlin tests** (tabs, omnibox, session, history + bookmarks incl. persistent file stores, downloads, settings) |
...
| Android shell UI (`src/android-app`) | Authored — Compose + Material 3 (browser, settings, downloads, history, bookmarks), bn/en strings; compiles in the Chromium build (B-001) |
| Patch framework | `iNWEB_PATCHES/` registry + apply/verify/hash tooling — tested |
| CI | **Live**: Python (30 tests) + registry + string parity; Kotlin core (171 tests, 2 modules); weekly upstream watch |
